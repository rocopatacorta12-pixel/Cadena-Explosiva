...
## v30 (2026-07-18)
- **A VER SI TE ACORDÁS**: el aviso del fantasma (👻 "NO toques el color X") ahora aparece SOLO, 2,6s ANTES de que empiece a mostrarse la secuencia (etapa nueva "aviso" + cartel grande `.mem-aviso-big`).
- **Último toque visible**: al completar la secuencia, el toque final se ve en verde en todos los celus durante 520ms (CFG.MEM_FIN_MS) y recién ahí pasa la bomba (guardas `_memDone` + `memIdx` contra toques extra en la ventana).
- **Colores más distintos**: paleta MEM re-tuneada (amarillo #fff200 puro vs naranja #ff6a00 profundo, rojo/azul/verde/violeta más saturados) + pads en reposo con opacity .5 y saturate(1.25).
